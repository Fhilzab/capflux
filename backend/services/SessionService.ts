...
/** Normalized identity exposed to handlers — never raw/refresh tokens. */
...
/** Runtime surface of WorkOS userManagement used by this service. */
interface UserManagementApi {
  authenticateWithSessionCookie(opts: { sessionData: string; cookiePassword: string }): Promise<{
    authenticated: boolean;
    user?: WorkosAuthenticationResponse['user'];
    sessionId?: string | null;
    organizationId?: string | null;
    authenticationMethod?: string | null;
    roles?: string[] | null;
  }>;
  sealSessionDataFromAuthenticationResponse(opts: {
...
class SessionService {
   ...
   */
  async createSessionCookieValue(authenticationResponse: WorkosAuthenticationResponse): Promise<string> {
    ...
    return this.um.sealSessionDataFromAuthenticationResponse({
      authenticationResponse: authenticationResponse as unknown as Record<string, unknown>,
   ...
   */
  async authenticateRequest({ sessionData, accessToken }: { sessionData?: string; accessToken?: string } = {}): Promise<NormalizedSession | null> {
    ...
    if (accessToken && !sessionData) {
      const result = await this.um.authenticateWithSessionCookie({
   ...
   */
  extractSession(req: Request): { sessionData?: string; accessToken?: string } {
   ...
   */
  parseCookieHeader(header?: string): Record<string, string> {
   ...
   */
  getLogoutUrl(sessionId: string, returnTo?: string): Promise<string> {
  ...
  async revokeSession(sessionId: string): Promise<void> {
    ...
    try {
      ...
      console.warn('Warn: failed to revoke session:', e?.message || err);
    }
  }
   ...
   */
  safeSessionPayload(session: Pick<NormalizedSession, 'user' | 'sessionId'> | null | undefined): { authenticated: false; user: null } | { authenticated: true; user: NormalizedSession['user']; sessionId: string | null } {
