...
test('PaymentGatewayInterface throws on unimplemented methods', async () => {
  ...
  const dynamicIface = iface as unknown as Record<string, (() => unknown) | undefined>;
  for (const method of syncMethods) {
    const fn = dynamicIface[method];
    assert.equal(typeof fn, 'function', `${method} missing on interface`);
    // noUncheckedIndexedAccess-safe: presence asserted above.
    assert.throws(() => { fn?.(); }, /must be implemented/);
  }
  for (const method of asyncMethods) {
    const fn = dynamicIface[method];
    assert.equal(typeof fn, 'function', `${method} missing on interface`);
    await assert.rejects(() => Promise.resolve(fn?.()), /must be implemented/);
  }
});
...
test('PaystackGateway implements all required methods', () => {
  ...
  const dynamicPaystack = gw as unknown as Record<string, unknown>;
  for (const method of requiredMethods) {
    assert.equal(typeof dynamicPaystack[method], 'function', `PaystackGateway missing ${method}`);
  }
});
...
test('PaystackGateway reports not configured when credentials absent', async () => {
  ...
  try {
    ...
    assert.match((err as Error).message, /Paystack is not configured/);
  }
});
...
test('TestGateway implements all required methods', async () => {
  ...
  const dynamicGw = gw as unknown as Record<string, unknown>;
  for (const method of requiredMethods) {
    assert.equal(typeof dynamicGw[method], 'function', `TestGateway missing ${method}`);
  }
});
