...
router.get('/:id', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('id, school_id, student_id, reference, gateway_txn_ref, amount_minor, currency, status, entry_category, payment_method, failure_reason, reversed_at, created_at, verified_at, students(first_name, last_name, class_name)')
      .eq('id', req.params.id)
      .eq('school_id', schoolId)
      .single();
...
router.post('/intent', requirePaymentReady, async (req, res) => {
  const { student_id, amount_minor, currency, reference, payment_method } = req.body || {};
...
router.post('/:id/reverse', requireStaff('payment.reconcile'), async (req, res) => {
  try {
    const schoolId = await getCallerSchool(req.user.id);
    if (!schoolId) return res.status(403).json({ error: 'No active school membership.' });

    const payment = await PaymentService.transition(req.params.id, schoolId, 'REVERSED', {
      actorId: req.user.id,
    });
