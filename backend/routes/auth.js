...
/**
 * Parse cookies from the request header using the project's existing
 * cookie utility (SessionService.parseCookieHeader) to avoid introducing
 * a second competing cookie parser.
 */

/**
 * GET /api/auth/authkit-url?mode=login|signup
 * Returns a WorkOS AuthKit authorization URL so the browser can redirect
 * to the WorkOS-hosted sign-in or sign-up screen.
 *
 * This endpoint is intentionally PUBLIC — unauthenticated users need it
 * to begin authentication. It never exposes WorkOS secrets, API keys,
 * or session cookies. It only returns a URL generated by the WorkOS SDK.
 * Sets an HttpOnly state cookie for CSRF protection on the callback.
 */
router.get('/authkit-url', (req, res) => {
  const mode = req.query.mode;
  if (mode !== 'login' && mode !== 'signup') {
    return res.status(400).json({ error: 'mode must be "login" or "signup"' });
  }
  try {
    const { url, state } = authService.getAuthKitAuthorizationUrl(mode);
