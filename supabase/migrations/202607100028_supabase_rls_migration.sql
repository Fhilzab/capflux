-- ============================================================
-- Migration: 028 — Supabase Auth RLS Policy Migration
-- ------------------------------------------------------------
-- Prerequisites:
--   - Migration 027 must have converted school_members.user_id
--     from TEXT to UUID.
--   - The handle_new_supabase_user() trigger (from 027) must be
--     active so auth.users → public.users provisioning works.
--
-- This migration replaces every auth.uid()::text reference with
-- auth.uid() (UUID) so that RLS policies align with the UUID
-- identity model after switching from WorkOS to Supabase Auth.
-- ============================================================
...
--    Was: USING (auth.uid()::text = id)
--    Now:  USING (auth.uid() = id)  -- UUID = UUID
-- ============================================================
...
--    Was: USING (auth.uid()::text = user_id)
--    Now:  USING (auth.uid() = user_id)
-- ============================================================
...
--    Was: USING (auth.uid()::text = user_id) WITH CHECK (auth.uid()::text = user_id)
--    Now:  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id)
-- ============================================================
...
--    Was: USING (auth.uid()::text IS NOT NULL)
--    Now:  USING (auth.uid() IS NOT NULL)
-- ============================================================
...
-- ============================================================
-- 5. public.school_members — "Users can view their own memberships"
-- ============================================================
...
--    (legacy profiles table — kept for backward compatibility)
-- ============================================================
...
--     Was: v_actor_id := auth.uid()::text;
--     Now:  v_actor_id := auth.uid();
-- ============================================================
...
-- ============================================================
-- Verify: Ensure no auth.uid()::text references remain.
-- (This is a documentation assertion — PostgreSQL does not
-- enforce SQL comments at runtime.)
-- ============================================================
