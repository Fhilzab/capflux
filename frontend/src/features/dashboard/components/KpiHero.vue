...
const secondaryStats = computed(() => [
  { label: 'Expected Collections', value: props.expectedCollections, format: 'currency' as const, icon: 'receipt', variant: 'revenue' },
  { label: 'Outstanding Balance', value: props.outstandingBalance, format: 'currency' as const, icon: 'exclamation-circle', variant: 'outstanding' },
  { label: 'Students Awaiting DVA', value: props.studentsAwaitingDva, format: 'number' as const, icon: 'wallet', variant: 'info' },
  { label: 'Pending Verification', value: props.pendingVerification, format: 'number' as const, icon: 'clock', variant: 'pending' },
]);
...
const iconColorClass = (variant: string) => {
  ...
  return colorMap[variant] || 'text-text-muted';
};
</script>
...
<template>
  <div class="space-y-4">
    ...
    <div class="premium-card p-6">
      <span class="accent-line accent-line--brand"></span>
      <div class="flex items-start justify-between">
        <div>
          <p class="text-label mb-2">Today's Collection</p>
          <p class="text-display text-brand mb-1">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
      <div v-for="stat in secondaryStats" :key="stat.label" class="premium-card p-4 transition-all duration-300 hover:shadow-elevated">
        <span class="accent-line" :class="`accent-line--${stat.variant}`"></span>
        <div class="flex items-start justify-between mb-2">
          <span class="text-xs text-label">{{ stat.label }}</span>
          <svg class="h-4 w-4" :class="iconColorClass(stat.variant)" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <p class="text-lg font-semibold font-mono" :class="iconColorClass(stat.variant)">
          {{ formattedValue(stat) }}
        </p>
