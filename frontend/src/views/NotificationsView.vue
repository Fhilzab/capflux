...
const submitNotification = async () => {
  ...
  form.value = { student_id: '', recipient_phone: '', message_body: '' };
};
  ...
  <main class="min-h-screen bg-slate-950 text-white p-8">
    <div class="max-w-6xl mx-auto space-y-6">
      <section class="rounded-3xl bg-slate-900 p-8 shadow-xl">
        ...
        <p class="text-slate-400">Create local notifications and track delivery status.</p>
      </section>
      ...
      <section class="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <div class="rounded-3xl bg-slate-900 p-8 shadow-xl">
          ...
          <p class="mt-4 text-slate-400">Notifications are stored locally and synced later.</p>
        </div>
      </section>
        ...
        <div class="grid gap-3">
          <div v-for="notification in notifications" :key="notification.id" class="rounded-2xl border border-slate-800 bg-slate-950 p-4">
            <div class="flex items-center justify-between gap-4">
              ...
              <span class="text-slate-400 text-sm">{{ notification.delivery_status }}</span>
            </div>
            <p class="mt-2 text-slate-400">{{ notification.message_body }}</p>
